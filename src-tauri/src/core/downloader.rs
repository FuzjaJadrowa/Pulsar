...
mod tests {
    ...
    fn sample_download_options() -> DownloadOptions {
        DownloadOptions {
            ...
            audio_quality: Some("192kbps".to_string()),
            is_time_range_active: false,
            ...
            download_subs: true,
            download_chat: false,
            subs_code: "en,pl,ja".to_string(),
            embed_subs: true,
            meta_sub_langs: vec!["en".to_string(), "pl".to_string()],
            meta_auto_langs: vec!["ja".to_string()],
            video_codec: Some("vp9 profile '0'".to_string()),
            audio_codec: Some("opus".to_string()),
            video_bitrate: Some("4500kbps".to_string()),
            audio_bitrate: Some("160kbps".to_string()),
            video_fps: Some("59.94fps".to_string()),
            audio_sample_rate: Some("48000Hz".to_string()),
            custom_args: None,
            client_task_id: None,
        }
    }
    ...
    #[test]
    fn quote_filter_value_escapes_and_quotes() {
        assert_eq!(quote_filter_value("av1"), "av1");
        assert_eq!(
            quote_filter_value("vp9 profile '0'"),
            "'vp9 profile \\'0\\''".to_string()
        );
    }

    #[test]
    fn build_video_and_audio_filters_parse_quality_values() {
        let options = sample_download_options();
        let video = build_video_filters(&options);
        let audio = build_audio_filters(&options);

        assert!(video.iter().any(|v| v == "vcodec='vp9 profile \\'0\\''"));
        assert!(video.iter().any(|v| v == "vbr=4500"));
        assert!(video.iter().any(|v| v == "fps=59.94"));
        assert!(audio.iter().any(|v| v == "acodec=opus"));
        assert!(audio.iter().any(|v| v == "abr=160"));
        assert!(audio.iter().any(|v| v == "asr=48000"));
    }

    #[test]
    fn build_subtitle_args_handles_known_manual_and_auto_langs() {
        let options = sample_download_options();
        let args = build_subtitle_args(&options);

        assert!(has_flag(&args, "--write-subs"));
        assert!(has_flag(&args, "--write-auto-subs"));
        assert!(has_flag(&args, "--embed-subs"));
        assert!(has_flag_value(&args, "--sub-langs", "en,pl,ja"));
    }

    #[test]
    fn parse_and_extract_time_values() {
        assert_eq!(parse_time_to_seconds("01:02:03"), Some(3723.0));
        assert_eq!(parse_time_to_seconds("1:2:3:4"), None);
        assert_eq!(
            extract_ffmpeg_elapsed_seconds(&json!({"out_time_ms": "6500"})),
            Some(6.5)
        );
        assert_eq!(
            extract_ffmpeg_elapsed_seconds(&json!({"out_time_us": 1_500_000})),
            Some(1.5)
        );
    }

    #[test]
    fn terminal_event_detection_matches_final_payloads() {
        assert!(is_terminal_event(&json!({"type": "finished"})));
        assert!(is_terminal_event(&json!({"status": "error"})));
        assert!(is_terminal_event(&json!({"event": "success"})));
        assert!(is_terminal_event(&json!({"success": false})));
        assert!(!is_terminal_event(&json!({"type": "progress"})));
    }

    #[test]
    fn guess_thumbnail_extension_prefers_content_type() {
        assert_eq!(
            guess_thumbnail_extension("image/webp", "https://x/y.jpg"),
            Some("webp".to_string())
        );
        assert_eq!(
            guess_thumbnail_extension("", "https://x/y.jpeg?token=1"),
            Some("jpg".to_string())
        );
    }
}*/